...
type
  ...
  public
    ...
    property includeattrib: fileattributesty
      Read fincludeattrib Write fincludeattrib default [fa_all];
    property excludeattrib: fileattributesty
      Read fexcludeattrib Write fexcludeattrib default [fa_hidden];
  ...
  published
    ...
    property optionsfile: filelistviewoptionsty
      Read foptionsfile Write setoptionsfile default defaultfilelistviewoptions;
...
type
  ...
  filedialogbeforeexecuteeventty = procedure(const Sender: tfiledialogxcontroller;
    var dialogkind: filedialogkindty; var aresult: modalresultty) of object;
  filedialogafterexecuteeventty = procedure(const Sender: tfiledialogxcontroller;
  ...
  public
    ...
    function Execute(var avalue: filenamety; const dialogkind: filedialogkindty;
      ...
      overload;
    function Execute(var avalue: filenamety; const dialogkind: filedialogkindty;
      const acaption: msestring; aoptions: filedialogoptionsty): boolean;
      overload;
  ...
  published
    ...
    property options: filedialogoptionsty
      Read foptions Write setoptions default defaultfiledialogoptions;
...
const
  defaultfiledialogoptionsedit1 =
    defaultoptionsedit1 + [oe1_savevalue, oe1_savestate, oe1_saveoptions];
...
type
  ...
  published
    ...
    property dialogkind: filedialogkindty
      Read fdialogkind Write fdialogkind default fdk_none;
    property optionsedit1: optionsedit1ty Read foptionsedit1
      Write foptionsedit1 default defaultfiledialogoptionsedit1;
...

{
  ...
  published
    ...
    property optionstree: treeitemeditoptionsty
      Read getoptionstree Write setoptionstree default [teo_treecolnavig,
      teo_enteronimageclick];
    property optionsedit: optionseditty Read getoptionsedit
      Write setoptionsedit default [oe_readonly, oe_undoonesc, oe_checkmrcancel,
    ...
    property onpathselected: dirtreepatheventty
      Read fonpathselected Write fonpathselected;
    property onselectionchanged: listitemeventty
  ...
  tfiledialogxfo = class(tmseform)
    ...
    procedure filteronsetvalue(const Sender: TObject; var avalue: msestring;
      var accept: boolean);
    ...
    procedure ondrawcellplacescust(const Sender: tcol; const Canvas: tcanvas;
      var cellinfo: cellinfoty);
    procedure oncellevcustplaces(const Sender: TObject; var info: celleventinfoty);
    procedure onmovesplit(const Sender: TObject);
    procedure onsetvalnoicon(const Sender: TObject; var avalue: boolean;
      var accept: boolean);
...
function filedialogx(var afilenames: filenamearty; const aoptions: filedialogoptionsty;
  const acaption: msestring;    //'' -> 'Open' or 'Save'
  const filterdesc: array of msestring; const filtermask: array of msestring;
  ...
  const imagelist: timagelist = nil; const ongetfileicon: getfileiconeventty = nil;
  const oncheckfile: checkfileeventty = nil): modalresultty;
  overload;
//threadsafe
function filedialogx(var afilename: filenamety; const aoptions: filedialogoptionsty;
  const acaption: msestring; const filterdesc: array of msestring;
  const filtermask: array of msestring; const adefaultext: filenamety = '';
...
function filedialogx1(dialog: tfiledialogxfo; var afilenames: filenamearty;
  const filterdesc: array of msestring; const filtermask: array of msestring;
  const filterindex: pinteger; const afilter: pfilenamety;      //nil -> unused
  const colwidth: pinteger;        //nil -> default
  const includeattrib: fileattributesty; const excludeattrib: fileattributesty;
  const history: pmsestringarty; const historymaxcount: integer;
  const acaption: msestring; const aoptions: filedialogoptionsty;
  const adefaultext: filenamety; const imagelist: timagelist;
  ...
  begin
    ...
    for int1 := 0 to high(filtermask) do
      if (int1 <= high(filterdesc)) and (filterdesc[int1] <> '') then
        filter.dropdown.cols[0][int1] :=
          filterdesc[int1] + ' (' + filtermask[int1] + ')'
      else
    ...
    if (afilter = nil) or (afilter^ = '') or (filter.dropdown.ItemIndex >= 0) and
...
function filedialogx(var afilenames: filenamearty; const aoptions: filedialogoptionsty;
  const acaption: msestring; const filterdesc: array of msestring;
  const filtermask: array of msestring; const adefaultext: filenamety = '';
  const filterindex: pinteger = nil; const filter: pfilenamety = nil;
  //nil -> unused
  const colwidth: pinteger = nil;        //nil -> default
  const includeattrib: fileattributesty = [fa_all];
  const excludeattrib: fileattributesty = [fa_hidden];
   ...
   {$endif}
    ...
    try
      Result := filedialogx1(dialog, afilenames, filterdesc, filtermask,
        filterindex, filter, colwidth, includeattrib, excludeattrib,
        history, historymaxcount, str1, aoptions, adefaultext, imagelist,
        ongetfileicon, oncheckfile);
    finally
      dialog.Free;
    end;
  finally
...
begin
  setlength(ar1, 1);
  ar1[0] := afilename;
  Result := filedialogx(ar1, aoptions, acaption, filterdesc, filtermask,
    adefaultext, filterindex, filter, colwidth, includeattrib,
    excludeattrib, history, historymaxcount, imagelist, ongetfileicon, oncheckfile);
...
begin
  ...
  begin
    foptionsfile := filelistviewoptionsty(setsinglebit(
{$ifdef FPC}
      longword
{$else}
      byte
...
{$else}
  if stringenter(mstr1, sc(sc_name), sc(sc_create_new_directory)) = mr_ok then
  begin
    places.defocuscell;
    places.datacols.clearselection;
    mstr1 := filepath(listview.directory, mstr1, fk_file);
...
procedure tfiledialogxfo.listviewitemevent(const Sender: tcustomlistview;
  const index: integer; var info: celleventinfoty);
var
  str1: filenamety;
begin
...
procedure tfiledialogxfo.listviewonkeydown(const Sender: twidget;
...
function tfiledialogxfo.tryreadlist(const adir: filenamety;
...
procedure tfiledialogxfo.filenamesetvalue(const Sender: TObject;
...
procedure tfiledialogxfo.dironsetvalue(const Sender: TObject;
  var avalue: mseString; var accept: boolean);
begin
  places.defocuscell;
  places.datacols.clearselection;
    ...
    begin
      ...
      begin
        if bnoicon.Value = True then
          ...
          tmp3 := ':';
        list_log[0][x] := tmp3 + tmp2 +
          msestring(filenamebase(listview.itemlist[x].Caption));
      ...
      {$else}
      list_log[3][x] := msestring(formatdatetime('YY-MM-DD hh:mm:ss',
        info.extinfo1.modtime));
      {$endif}
...
procedure tfiledialogxfo.filteronsetvalue(const Sender: TObject;
  var avalue: msestring; var accept: boolean);
var
  rootdir: msestring;
  bool: boolean;
...
begin
  ...
  if (filename.Value <> '') or (fdo_acceptempty in dialogoptions) or
    (filename.tag = 1) then
  begin
    ...
    if (fdo_checkexist in dialogoptions) and not
      ((filename.Value = '') and (fdo_acceptempty in dialogoptions)) then
    begin
      if fdo_directory in dialogoptions then
...
{$else}
          if not askok(sc(sc_file) + ' "' + tosysfilepath(filenames[0]) +
            '" ' + sc(sc_exists_overwrite), sc(sc_warningupper)) then
          begin
            filename.SetFocus;
            Exit;
          end;
...
procedure tfiledialogxfo.showhiddenonsetvalue(const Sender: TObject;
...
procedure tfiledialogxfo.ondrawcell(const Sender: tcol; const Canvas: tcanvas;
  var cellinfo: cellinfoty);
var
  aicon:  integer;
  apoint: pointty;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (trim(list_log[1][cellinfo.cell.row]) = '') and
        (trim(list_log[2][cellinfo.cell.row]) = '') then
        aicon := 0
        ...
        aicon := 4
      else if (lowercase(list_log[1][cellinfo.cell.row]) = '.ico') or
...
procedure tfiledialogxfo.onsetcomp(const Sender: TObject; var avalue: boolean;
  var accept: boolean);
var
  theint, theexist: integer;
  sel: gridcoordty;
begin
  ...
  begin
    ...
    for theint := 0 to list_log.rowcount - 1 do
      if trim(copy(list_log[0][theint], 2, length(list_log[0][theint]))) =
        filename.Value then
        theexist := theint;
...
procedure tfiledialogxfo.oncellevplaces(const Sender: TObject;
...
procedure tfiledialogxfo.ondrawcellplace(const Sender: tcol;
...
procedure tfiledialogxfo.onlateral(const Sender: TObject; var avalue: boolean;
  var accept: boolean);
begin
  if not avalue then
  begin
...
procedure tfiledialogxfo.ondrawcellplacescust(const Sender: tcol;
  const Canvas: tcanvas; var cellinfo: cellinfoty);
var
  aicon:  integer;
  apoint: pointty;
...
procedure tfiledialogxfo.oncellevcustplaces(const Sender: TObject;
  var info: celleventinfoty);
var
  theint, theexist: integer;
  ...
  if (info.eventkind = cek_buttonrelease) or (info.eventkind = cek_keyup) then
    ...
    begin
      ...
      begin
        ...
        if directoryexists(tosysfilepath(placescust[1][info.cell.row] +
...
procedure tfiledialogxfo.onsetvalnoicon(const Sender: TObject;
  var avalue: boolean; var accept: boolean);
var
  tmp: msestring;
  x: integer;
...
constructor tfiledialogxcontroller.Create(const aowner: tmsecomponent = nil;
  const onchange: proceventty = nil);
begin
...
function tfiledialogxcontroller.Execute(dialogkind: filedialogkindty;
  const acaption: msestring; aoptions: filedialogoptionsty): modalresultty;
var
  po1: pmsestringarty;
  fo: tfiledialogxfo;
...
begin
 ...
 {$ifdef windows}
  for achar := 'A' to 'Z' do
    ...
    if directoryexists(tosysfilepath(sys_getuserhomedir + directoryseparator +
      'Desktop')) then
    begin
      Inc(x);
      fo.places.rowcount := x + 1;
...
{$endif}
    ...
    if (fwindowrect.cx > 0) and (fwindowrect.cy > 0) then
      fo.widgetrect :=
        clipinrect(fwindowrect, application.screenrect(fo.window));
    ...
    Result := filedialogx1(fo, ffilenames, ara, arb, @ffilterindex,
      @ffilter, @fcolwidth, finclude, fexclude, po1, fhistorymaxcount,
      acaption, aoptions, fdefaultext, fimagelist, fongetfileicon, foncheckfile);
...
function tfiledialogxcontroller.Execute(const dialogkind: filedialogkindty;
  const acaption: msestring): modalresultty;
begin
  Result := Execute(dialogkind, acaption, foptions);
end;

function tfiledialogxcontroller.actcaption(
...
begin
  if fdo_directory in aoptions then
    Result := Execute(dialogkind, fcaptiondir, aoptions)
  else
    Result := Execute(dialogkind, actcaption(dialogkind), aoptions);
end;

function tfiledialogxcontroller.Execute(dialogkind: filedialogkindty =
  fdk_none): modalresultty;
begin
  if dialogkind = fdk_none then
  ...
  if fdo_directory in foptions then
    Result := Execute(dialogkind, fcaptiondir)
  else
    Result := Execute(dialogkind, actcaption(dialogkind));
end;

function tfiledialogxcontroller.Execute(var avalue: filenamety;
  dialogkind: filedialogkindty = fdk_none): boolean;
begin
  if dialogkind = fdk_none then
    if fdo_save in foptions then
  ...
  if fdo_directory in foptions then
    Result := Execute(avalue, dialogkind, fcaptiondir)
  else
...
{$else}
  Result := not findfile(filename) or askok(sc(sc_file) +
    ' "' + filename + '" ' + sc(sc_exists_overwrite), sc(sc_warningupper));
{$endif}
end;

function tfiledialogxcontroller.Execute(var avalue: filenamety;
  const dialogkind: filedialogkindty; const acaption: msestring): boolean;
begin
...
begin
  ...
  begin
    ...
    akind := fk_file;
  if [fdo_relative, fdo_lastdirrelative, fdo_basedirrelative] * foptions <> [] then
  begin
    if fdo_relative in foptions then
      flastdir := getcurrentdirmse
    else
  ...
  begin
    ...
    for int1 := 0 to high(ffilenames) do
      ffilenames[int1] := filepath(filenames[int1], akind, not
        (fdo_absolute in foptions));
  end;
end;
...
begin
  Value := filedialogoptionsty(setsinglebit(card32(Value), card32(foptions),
    [card32([fdo_absolute, fdo_relative, fdo_lastdirrelative, fdo_basedirrelative]),
...
function tfiledialogx.Execute(const akind: filedialogkindty;
  const aoptions: filedialogoptionsty): modalresultty;
begin
